	...
	useEffect(()=>{
		async function start(){
			const response = await apiFetch(
					{
						path: `/wp/v2/mo_persons/${attributes.personId}`,
						method: 'GET',
					}
				)
		...
		<div className='person_container'>
			<div className="person_select_container">
				<select onChange={(e) => setAttributes({personId: e.target.value}) }>
					...
					{setAttributes({firstName:preview.first_name})}
					{setAttributes({lastName:preview.last_name})}
					{setAttributes({position:preview.position})}
					{setAttributes({image:preview.image})}
					{setAttributes({github:preview.github})}
					{setAttributes({linkedin:preview.linkedin})}
					{setAttributes({xing:preview.xing})}
					{setAttributes({facebook:preview.facebook})}
			</div>
			{attributes.personId && (				
				<div className="person_selected_preview">
				<div className="person_container_content">
				<div className="mo_person_full_name">
					{`${preview.first_name} ${preview.last_name}`}
				</div>
				<div className="mo_person_description">
					{preview.description}
				</div>
				<div className="mo_person_image">
					<img src={preview?.image} width={150} height={150} alt={`Picture of ${preview?.first_name} ${preview?.last_name}`}/>
				</div>
				<div className="mo_person_position">
						{preview?.position}
				</div>
				<ul className="mo_person_social_media">
					<li className="social_item_list mo_person_github_link">
						<a target="_blank" className="social_link" href={preview?.github}>
					...
					<li className="social_item_list mo_person_linkedin_link">
						<a target="_blank" className="social_link" href={preview.linkedin}>
					...
					<li className="social_item_list mo_person_xing_link">
						<a target="_blank" className="social_link" href={preview?.xing}>
					...
					<li className="social_item_list mo_person_facebook_link">
						<a target="_blank" className="social_link" href={preview?.facebook}>
